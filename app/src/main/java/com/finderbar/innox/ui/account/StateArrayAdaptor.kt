...
class StateArrayAdaptor(mContext: Context, private val textViewResourceId: Int, val arrays: MutableList<State>) : ArrayAdapter<State>(mContext, 0, arrays) {
    ...
    private val filterName = object : Filter() {
        override fun convertResultToString(resultValue: Any?): CharSequence {
            val state: State = resultValue as State
        ...
        override fun performFiltering(constraint: CharSequence?): FilterResults {
            ...
            if (constraint != null) {
                val suggestions: ArrayList<State> = ArrayList<State>()
                for (arr in arrays) {
        ...
        override fun publishResults(constraint: CharSequence?, results: FilterResults?) {
            ...
            if (results != null && results.count > 0) {
                addAll((results.values as ArrayList<State?>))
            } else {
                addAll(arrays)
            }
